  ...
  suite('Routing tests', function() {
    ...
    suite('POST /api/books with title => create book object/expect book object', function() {
      ...
      test('Test POST /api/books with title', function(done) {
        ...
        .end(function(err, res){
          assert.equal(res.status, 200);
          assert.equal(res.body.title, 'test title 2');
          assert.property(res.body, '_id', 'Books in array should contain _id');
        });
      ...
      test('Test GET /api/books',  function(done){
        ...
        .end(function(err, res){
          ...
          res.body.forEach(function(book){
            ...
            assert.property(book, 'comments', 'Books in array should contain comments');
            assert.property(book, '__v', 'Books in array should contain __v');
            assert.isArray(book.comments, 'Books in array should contain comments');
          })
          done();
        });
      });      
    ...
    suite('POST /api/books/[id] => add comment/expect book object with id', function(){
      ...
      test('Test POST /api/books/[id] without comment field', function(done){
        chai.request(server)
        .keepOpen()
        .post('/api/books/c1bcd4300c9240868118384fd9dcf9b8')
        .send({})
        .end(function(err, res){
          assert.equal(res.status, 200);
